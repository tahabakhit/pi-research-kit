...
X recent search, YouTube Data API, and Bluesky adapters are opt-in and require credentials. X recent search is limited to the provider's recent-search window. YouTube metadata and top-level comment retrieval use the YouTube Data API. An optional trusted, operator-configured `yt-dlp` executable can retrieve available captions without cookies, media downloads, or automatic installation. Bluesky search authenticates with `com.atproto.server.createSession` at `bsky.social`, keeps the access token in process memory only, and re-creates the session once when the token is rejected. It queries `app.bsky.feed.searchPosts` sorted by latest, bounded to 25 posts, within the requested date window. Evidence uses the `bsky.app` post URL, author handle, post text, and the post creation date (indexed date when absent); likes and replies map to engagement score and comments. No paid API is enabled without explicit operator opt-in, and provider quotas or charges remain the operator's responsibility.
...
- `PI_RESEARCH_ENABLE_BLUESKY=1`, `PI_RESEARCH_BLUESKY_HANDLE`, and `PI_RESEARCH_BLUESKY_APP_PASSWORD` — opt in to Bluesky post search. Use an app password, not the account password.
- `PI_RESEARCH_KETCH_CONFIG` — optional absolute path to an existing Ketch `config.json`, passed to Ketch as `KETCH_CONFIG`. The file must be readable; its contents are never read or logged by this package.
- `PI_RESEARCH_KETCH_BACKEND` — optional default search backend for `ketch_search`, `web_search`, and research workflows: one of `brave`, `ddg`, `searxng`, `exa`, `firecrawl`, `keenable`, `tavily`, `parallel`, `serpbase`, `degoog`, or `multi:<comma list of those names>` (for example `multi:firecrawl,ddg`). Other values are rejected. A backend, `multi`, or `random` argument supplied to a tool call takes precedence.

The Ketch launcher runs shell-free with bounded output and time in a fresh temporary home/config/cache environment. Existing Ketch configuration, API keys, and cookies are not reused automatically. With `PI_RESEARCH_KETCH_CONFIG` set, Ketch reads that one config file (including any provider keys and browser path it contains), while HOME and the cache remain temporary. Without it, Ketch uses its built-in defaults. The default package has no authenticated-provider setup UI.
